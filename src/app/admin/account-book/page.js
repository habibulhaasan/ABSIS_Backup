...
function LedgerRow({row, isOpen, onToggle, isGrouped}) {
  ...
  // Split credits: entry_fee / reregistration_fee → Fees column; everything else → Capital column
  const isFeeRow = !isGrouped && (row.type==='entry_fee' || row.type==='reregistration_fee');
  const capitalCredit = isFeeRow ? 0 : (row.credit||0);
  const feeCredit     = isFeeRow ? (row.credit||0) : 0;
  // For grouped rows split by summing entries
  const groupCapital  = isGrouped
    ? (row.entries||[]).filter(e=>e.type!=='entry_fee'&&e.type!=='reregistration_fee').reduce((s,e)=>s+e.credit,0)
    : 0;
  const groupFees     = isGrouped
    ? (row.entries||[]).filter(e=>e.type==='entry_fee'||e.type==='reregistration_fee').reduce((s,e)=>s+e.credit,0)
    : 0;
  return (
    <>
      <div onClick={onToggle}
        style={{display:'grid',gridTemplateColumns:'0.9fr 0.5fr 2fr 0.9fr 0.9fr 0.9fr 0.6fr 1fr 0.4fr',
        ...
        <div style={{textAlign:'right',fontWeight:600,fontSize:13,
          color:(isGrouped?groupCapital:capitalCredit)>0?'#15803d':'#e2e8f0'}}>
          {(isGrouped?groupCapital:capitalCredit)>0 ? fmt(isGrouped?groupCapital:capitalCredit) : '—'}
        </div>
        {/* Fee credit */}
        <div style={{textAlign:'right',fontWeight:600,fontSize:13,
          color:(isGrouped?groupFees:feeCredit)>0?'#d97706':'#e2e8f0'}}>
          {(isGrouped?groupFees:feeCredit)>0 ? fmt(isGrouped?groupFees:feeCredit) : '—'}
        </div>
        {/* Inst count */}
        <div style={{textAlign:'right',fontSize:12,color:'#64748b'}}>
          {row.count>0 ? (isGrouped?row.count:1) : '—'}
        </div>
        {/* Balance */}
              ...
              {filteredEntries.length === 0 ? (
                ...
                <div style={{borderRadius:12,border:'1px solid #e2e8f0',overflow:'hidden'}}>
                  <div style={{display:'grid',
                    gridTemplateColumns:'0.9fr 0.5fr 2fr 0.9fr 0.9fr 0.9fr 0.6fr 1fr 0.4fr',
                    padding:'9px 16px',background:'#0f172a'}}>
                    {['Date','Type','Description','Debit (−)','Capital (+)','Fees (+)','Inst.','Balance',''].map(h => (
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:16}}>
                {FUNDS.map(({key:fKey, ...fProps}) => <FundCard key={fKey} {...fProps}/>)}
                  ...
                  <div style={{display:'grid',
                    ...
                    <div style={{textAlign:'right',fontWeight:700,color:'#fde68a'}}>
                      {(() => { const t=filteredEntries.filter(e=>e.type==='entry_fee').reduce((s,e)=>s+e.credit,0); return t>0?fmt(t):'—'; })()}
                    </div>
                    <div style={{textAlign:'right',fontWeight:700,color:'#93c5fd'}}>{instCount}</div>
                    <div style={{textAlign:'right',fontWeight:800,fontSize:15,color:'#fff'}}>
                      {fmt(ledgerBalance)}
                    ...
                    <div/>
                  </div>book/page.js — Finance Hub (Phase 6)
'use client';
import { useState, useEffect, useMemo } from 'react';
import { db } from '@/lib/firebase';
import { collection, getDocs, query, orderBy, doc, getDoc } from 'firebase/firestore';
