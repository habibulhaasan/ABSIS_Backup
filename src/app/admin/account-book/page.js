...
function LedgerRow({row, isGrouped}) {
  ...
  return (
    <div
      style={{display:'grid',gridTemplateColumns:'0.85fr 2.5fr 1fr 1fr 1fr 0.5fr 1fr',
        padding:'10px 16px',
      ...
      <div style={{fontSize:12,color:'#475569',whiteSpace:'nowrap'}}>
        {isGrouped ? row.label : row.date}
      </div>
      {/* Type badge + Description merged */}
      <div style={{minWidth:0}}>
        <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
          {!isGrouped && <TypeBadge type={row.type}/>}
          <span style={{fontWeight:600,fontSize:13,color:'#0f172a',
            overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
            {isGrouped ? `${row.entries.length} transactions` : row.desc}
          </span>
        </div>
        {!isGrouped && row.sub && (
          <div style={{fontSize:11,color:'#94a3b8',overflow:'hidden',
            textOverflow:'ellipsis',whiteSpace:'nowrap',marginTop:2}}>{row.sub}</div>
        )}
        {isGrouped && (
